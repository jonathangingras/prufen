...
sweetgreen_test_define(expect_equal_memory, positive_comparison_works) {
  sweetgreen_expect_equal_memory(buffer1, buffer2, strlen(buffer1) + 1);
}

sweetgreen_test_define(expect_not_equal_memory, negative_comparison_works) {
  sweetgreen_expect_not_equal_memory(buffer1, not_equal_buffer, strlen(buffer1) + 1);
}

sweetgreen_test_define(expect_equal_memory_on_heap, positive_comparison_works) {
...
sweetgreen_test_define(expect_not_equal_memory_on_heap, negative_comparison_works) {
  char *_buffer1 = malloc(strlen(buffer1) + 1);
  memcpy(_buffer1, buffer1, strlen(buffer1) + 1);
