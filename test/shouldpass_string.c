...
pruf_test_define(expect_equal_memory, positive_comparison_works) {
  pruf_expect_equal_string(buffer1, buffer2);
}

pruf_test_define(expect_not_equal_string, negative_comparison_works) {
  pruf_expect_not_equal_string(buffer1, not_equal_buffer);
}

pruf_test_define(expect_equal_string_on_heap, positive_comparison_works) {
...
pruf_test_define(expect_not_equal_string_on_heap, negative_comparison_works) {
  char *_buffer1 = malloc(strlen(buffer1) + 1);
  memcpy(_buffer1, buffer1, strlen(buffer1) + 1);
