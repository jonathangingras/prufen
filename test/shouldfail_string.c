...
sweetgreen_test_define(failing_expect_equal_memory, positive_comparison_fails) {
  sweetgreen_expect_equal_string(buffer1, not_equal_buffer);
}

sweetgreen_test_define(failing_expect_not_equal_string, negative_comparison_fails) {
  sweetgreen_expect_not_equal_string(buffer1, buffer2);
}

sweetgreen_test_define(failing_expect_equal_string_on_heap, positive_comparison_fails) {
...
sweetgreen_test_define(failing_expect_not_equal_string_on_heap, negative_comparison_fails) {
  char *_buffer1 = malloc(strlen(buffer1) + 1);
  memcpy(_buffer1, buffer1, strlen(buffer1) + 1);
