...
pruf_test_define(failing_expect_equal_memory, positive_comparison_fails) {
  pruf_expect_equal_memory(buffer1, not_equal_buffer, strlen(buffer1) + 1);
}

pruf_test_define(failing_expect_not_equal_memory, negative_comparison_fails) {
  pruf_expect_not_equal_memory(buffer1, buffer2, strlen(buffer1) + 1);
}

pruf_test_define(failing_expect_equal_memory_on_heap, positive_comparison_fails) {
...
pruf_test_define(failing_expect_not_equal_memory_on_heap, negative_comparison_fails) {
  char *_buffer1 = malloc(strlen(buffer1) + 1);
  memcpy(_buffer1, buffer1, strlen(buffer1) + 1);
